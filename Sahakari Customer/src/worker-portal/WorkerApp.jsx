...
const RIGHT_TABS = [
  ...
  { id: 'LEADS', label: 'Live Leads', icon: 'radar' },
];

const NEGOTIATION_NAVS = ['negotiations', 'cooperative'];

function WorkerPortalMain() {
  ...
  return (
    <div className="min-h-screen bg-surface-dim/20 text-on-surface flex flex-col font-sans">
      {/* Top Header */}
      <Header
          ...
          <div className="w-full max-w-[1280px] flex flex-col lg:flex-row gap-0 lg:gap-6 p-0 lg:p-6 min-h-[calc(100vh-112px)]">
            ...
            {isNegotiationView && (
              <>
                ...
                <main className="flex-1 flex flex-col bg-surface border-x lg:border border-outline-variant lg:rounded-2xl overflow-hidden shadow-elevation-2 h-[560px] lg:h-[640px]">
                  <JobContextHeader
                    onOpenPhotos={() => {
            ...
            {/* ── OTHER NAV VIEWS (Profile & Modules) ── */}
            {!isNegotiationView && (
